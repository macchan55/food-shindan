...
### 診断（`/diagnosis`）
- 64問・16シーン構成（全シーン4問ずつ）。質問ごとに専用イラストあり。元は20シーン
  （シーン13〜20は2問ずつ）だったが、後半の短いシーンでシーン切替の演出（下記）が
  頻発して煩わしいという指摘を受け、migration 0009で隣接する2問シーンをペアで統合し、
  全シーンを4問に揃えた（scene_id/scene_titleの張り替えのみ。設問内容・スコアリングは
  無変更）
- 回答はシーン単位で1ページ。シーン内は回答するたびに次の設問へ自動スクロールし、
  ...
  遷移が重かったための変更）
- 新しいシーンに前進した時だけ、そのシーンのタイトル・最初の設問イラスト・問題数を
  見せる章扉インタースティシャル（`このシーンをはじめる`ボタンでタップ進行）を挟む。
  64問を1本の連続スクロールに感じて単調という指摘を受けて追加。「前の質問に戻る」で
  シーン途中/末尾に戻った時や同一シーン内の遷移では表示しない
  （`src/app/diagnosis/[sessionId]/page.tsx`の`dismissedIntros`）
